#!/usr/bin/env python3
"""Write quiz-paintings.js using verified Commons cache + Wikipedia thumbnails."""
...
# Quiz title -> English Wikipedia article (for thumbnail lookup).
WIKI_TITLES = {
...
def wiki_thumb(article):
    ...
    for attempt in range(6):
        try:
            ...
            if err.code in (429, 503) and attempt < 5:
...
def main():
    ...
    print(f"Starting from {len(merged)} cached Commons URLs")

    for quiz_title, wiki_title in WIKI_TITLES.items():
        if quiz_title in merged:
            continue
        image = wiki_thumb(wiki_title)
        if image:
            merged[quiz_title] = image
            print(f"ok: {quiz_title}")
        else:
            print(f"skip: {quiz_title}")
        time.sleep(3)

    CACHE.write_text(json.dumps(merged, ensure_ascii=False, indent=2), encoding="utf-8")

    order = list(json.loads(CACHE.read_text(encoding="utf-8")).keys())
    paintings = [{"title": title, "image": merged[title]} for title in order if title in merged]
    write_js(paintings)
    print(f"Wrote {len(paintings)} paintings")
